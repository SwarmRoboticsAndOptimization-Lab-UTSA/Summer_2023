...
#opening a file to save data to
file = open("gyro_radians.txt", mode="wt")
...
def new_client(client_index, client_sock, client_addr):
	...
	while True:
		count += 1
		start_time = time.time() #reseting the time at the beginnnig of every loop to then use in the heading calculation
		...
		while(expected_recv_packets > 0):
			...
			if header == bytearray([1]): # Get a QQVGA image
				...
				left_des_speed = 1000
				right_des_speed = -1000
				""" random movement used for heading demo
				if count > 100:
					left_des_speed = random.randrange(-250, 250)
					right_des_speed = random.randrange(-250,250)
					count = 0
				"""
				LLSB = left_des_speed & 0xFF
				LMSB = (left_des_speed>>8) & 0xFF
				RLSB= right_des_speed & 0xFF
				RMSB = (right_des_speed>>8) & 0xFF
				#command[client_index][2] = 1    # Settings: set motors speed
				command[client_index][3] = LLSB # left motor LSB
				command[client_index][4] = LMSB # left motor MSB
				command[client_index][5] = RLSB   # right motor LSB
				command[client_index][6] = RMSB  # right motor MSB

				#calculate heading in radians
				heading = heading + (math.pi*.002*.0205*(right_des_speed-left_des_speed) * (time.time()-start_time))/.052
				heading = heading % (math.pi*2)
				#print(str(heading / math.pi) + "π")

				# converting the zlsb and zmsb of the GYROSCOPE into one value using the inverse of the motor calculation
				gyro_ZLSB_binary = format(sensor[22], 'b')
				while len(gyro_ZLSB_binary) < 9:
					gyro_ZLSB_binary = "0" + gyro_ZLSB_binary
				binary_gyro = format(sensor[23], 'b') + gyro_ZLSB_binary
				gyro_value = int(binary_gyro, 2)
				if left_des_speed > right_des_speed:
					gyro_value = gyro_value - 130546				
				#will give you angular velocity values between (-65275, 65276)

				
				### Math to convert given gyroscope values to radians per second
				gyro_value_radians = ((gyro_value) / 65276) * 4.860577313 #this constant is the theoretical maximum angular velocity of the robot in radians/second
				#print(gyro_value_radians)
				
				### used to collect the data in a file to graph
				
				if count < 400:
					file.write(str(gyro_value_radians) + "\n")
				elif count == 400:
					file.close()
					print("closed")
				
				
				# End of Sam's controller #############################################################################################################
